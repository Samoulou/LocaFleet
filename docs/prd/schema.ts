...
export const tenants = pgTable("tenants", {
  ...
  settings: jsonb("settings").default({}), // Tarifs par défaut, email mécanicien, etc.
...
export const vehicles = pgTable(
  "vehicles",
  {
    ...
    notes: text("notes"),
    // Daily override rates (null = use category default)
...
export const clients = pgTable(
  "clients",
  {
    ...
    // Company (optional, for B2B clients)
...
export const rentalOptions = pgTable(
  "rental_options",
  {
    ...
    tenantId: uuid("tenant_id")
      .notNull()
      .references(() => tenants.id, { onDelete: "cascade" }),
    name: varchar("name", { length: 100 }).notNull(), // GPS, Siège bébé, etc.
    dailyPrice: decimal("daily_price", { precision: 10, scale: 2 }).notNull(),
    isPerDay: boolean("is_per_day").default(true), // true = per day, false = flat fee
...
export const rentalContracts = pgTable(
  "rental_contracts",
  {
    ...
    tenantId: uuid("tenant_id")
      .notNull()
      .references(() => tenants.id, { onDelete: "cascade" }),
    contractNumber: varchar("contract_number", { length: 50 }).notNull(), // e.g. #LF-2026-042
    clientId: uuid("client_id")
      .notNull()
...
export const inspectionPhotos = pgTable(
  "inspection_photos",
  {
    ...
    url: text("url").notNull(),
    fileName: varchar("file_name", { length: 255 }),
    position: photoPositionEnum("position"), // front, back, left, right, other (V1.1)
...
export const invoices = pgTable(
  "invoices",
  {
    ...
    invoiceNumber: varchar("invoice_number", { length: 50 }).notNull(), // e.g. #INV-2026-001
    clientId: uuid("client_id")
      .notNull()
      .references(() => clients.id),
    ...
    taxRate: decimal("tax_rate", { precision: 5, scale: 2 }).default("0"), // TVA % if applicable
    taxAmount: decimal("tax_amount", { precision: 10, scale: 2 }).default("0"),
    totalAmount: decimal("total_amount", { precision: 10, scale: 2 }).notNull(),
    // Line items stored as JSONB for flexibility
    // Format: [{ description, quantity, unitPrice, totalPrice }]
...
export const payments = pgTable(
  "payments",
  {
    ...
    reference: varchar("reference", { length: 255 }), // Référence paiement
...
export const rentalDossiers = pgTable(
  "rental_dossiers",
  {
    ...
    // Snapshot data for archive
    clientName: varchar("client_name", { length: 255 }).notNull(),
    vehicleInfo: varchar("vehicle_info", { length: 255 }).notNull(), // "BMW X3 - VD 112 233"
    rentalPeriod: varchar("rental_period", { length: 100 }).notNull(), // "01 Oct - 05 Oct 2026"
    totalAmount: decimal("total_amount", { precision: 10, scale: 2 }).notNull(),
    // PDF links (all docs in one place)
    contractPdfUrl: text("contract_pdf_url"),
...
export const emailLogs = pgTable(
  "email_logs",
  {
    ...
    body: text("body"), // HTML content or template reference
    // Related entities
...
export const notifications = pgTable(
  "notifications",
  {
    ...
    linkUrl: varchar("link_url", { length: 500 }), // Internal link to relevant page
...
export const auditLogs = pgTable(
  "audit_logs",
  {
    ...
    entityType: varchar("entity_type", { length: 50 }).notNull(), // vehicle, client, contract, invoice, etc.
