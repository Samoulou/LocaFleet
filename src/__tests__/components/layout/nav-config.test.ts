...
describe("nav-config", () => {
  ...
  it("getVisibleNavItems('admin') returns all 8 items", () => {
    const items = getVisibleNavItems("admin");
    expect(items).toHaveLength(8);
  });

  it("getVisibleNavItems('agent') returns 7 items (no settings)", () => {
    const items = getVisibleNavItems("agent");
    expect(items).toHaveLength(7);
    expect(items.find((i) => i.key === "settings")).toBeUndefined();
  });

  it("getVisibleNavItems('viewer') returns 7 items (no settings)", () => {
